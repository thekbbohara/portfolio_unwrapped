...
export const transactionsSchema = z.object({
  buyTransactions: z.number().min(0),
  sellTransactions: z.number().min(0),
  rocket: rocketSchema,
});

export const ISSUES_EXIT_DURATION = 20;

export const getTransactionsDuration = ({
  buyTransactions,
  sellTransactions,
}: {
  ...
  return 4 * VIDEO_FPS + 20 + getTotalShootDuration(sellTransactions);
};

export const calculateTransactionsDuration: CalculateMetadataFunction<
  z.infer<typeof transactionsSchema>
> = ({ defaultProps: { buyTransactions, sellTransactions } }) => {
  return {
    durationInFrames: getTransactionsDuration({
      buyTransactions,
      sellTransactions,
    }),
  };
...
export const getTransactionsAssetsToPrefetch = () => {
  return [...getIssueSounds(), UFO_ASSET];
};

export const Transactions: React.FC<z.infer<typeof transactionsSchema>> = ({
  buyTransactions,
  sellTransactions,
  rocket,
}) => {
  ...
  const {
    ...
    return makeUfoPositions({
      totalUfos: totalTransactions,
      closedIssues: sellTransactions,
    });
  }, [sellTransactions, totalTransactions]);
  ...
  const UFO_EXIT_START =
    TIME_BEFORE_SHOOTING + getTotalShootDuration(sellTransactions) + 10;
  ...
  return (
    <AbsoluteFill>
      ...
      <AbsoluteFill style={{ transform: `translateY(${yOffset}px)` }}>
        ...
        {totalTransactions === 0 ? <ZeroIssues /> : false}
        {ufos.map((p, i) => {
          const explosion = explosions.find((e) => e.index === i);
          return (
            <Sequence
